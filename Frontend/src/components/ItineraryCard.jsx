...
export default function Itinerary() {
  // Read raw AI itinerary from localStorage and normalize to the shape this component expects.
  let rawStorage = null;
  try {
    rawStorage = JSON.parse(localStorage.getItem("aiItinerary"));
  } catch (e) {
    console.warn("Itinerary parse error:", e);
  }
  ...
  const data = React.useMemo(() => {
    ...
    // Hotels fallback
    const hotels = raw.hotels || raw.hotels_recommendations || { budget: [], mid: [], luxury: [] };

    // Transport / flights
    const flight_cost = raw.flights_trains?.flight_cost || raw.transport?.flight_cost || raw.transport?.flight || raw.total_cost?.flight || raw.transport?.flight_cost || "N/A";
    const local_transport = raw.local_transport || raw.flights_trains?.local_transport || raw.transport?.local_transport || raw.transport?.local || "N/A";

    // Food cost
    const food_cost = raw.food_guide?.average_food_cost_per_day || raw.food_cost || raw.total_cost?.food || raw.total_cost?.food_total || "N/A";

    // Pocket money
    const pocket_money = raw.pocket_money || raw.total_cost?.pocket_money || raw.pocket || "N/A";

    // Total estimate normalization
    let total_estimate = { breakdown: [], total: raw.total_cost?.total || raw.total_estimate?.total || raw.total || "N/A" };
    if (Array.isArray(raw.total_cost?.breakdown)) {
      total_estimate.breakdown = raw.total_cost.breakdown;
    } else if (Array.isArray(raw.total_estimate?.breakdown)) {
      total_estimate.breakdown = raw.total_estimate.breakdown;
    } else if (raw.total_cost && typeof raw.total_cost === "object") {
      // convert object to breakdown array (skip total key)
      total_estimate.breakdown = Object.entries(raw.total_cost)
        .filter(([k]) => k !== "total")
        .map(([k, v]) => ({ item: k, cost: v }));
    }

    // Safety tips
    const tips = raw.safety_tips || raw.safety || raw.tips || raw.safetyTips || "";

    // Packing list
    const packing_list = Array.isArray(raw.packing_list) ? raw.packing_list.join("\n") : raw.packing_list || "";

    // Daywise itinerary must be an array; otherwise provide an empty array
    const daywise_itinerary = Array.isArray(raw.daywise_itinerary) ? raw.daywise_itinerary : Array.isArray(raw.itinerary) ? raw.itinerary : [];

    return {
      trip_summary: raw.trip_summary || raw.summary || "",
      best_time: raw.best_time || raw.best_months || "",
      weather: raw.weather || "",
      daywise_itinerary,
      hotels,
      transport: { flight_cost, local_transport },
      food_cost,
      pocket_money,
      total_estimate,
      tips,
