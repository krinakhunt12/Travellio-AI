...
export default function HotelsPage() {
  ...
  const hotelsList = [];
  if (tripData?.hotels) {
    // combine budget/mid/luxury into flat list with category
    const budgets = (tripData.hotels.budget || []).map((h, i) => ({ ...h, category: 'budget', id: `budget-${i}` }));
    const mids = (tripData.hotels.mid || []).map((h, i) => ({ ...h, category: 'mid', id: `mid-${i}` }));
    const lux = (tripData.hotels.luxury || []).map((h, i) => ({ ...h, category: 'luxury', id: `lux-${i}` }));
    hotelsList.push(...budgets, ...mids, ...lux);
  }

  const filtered = filter === "all" ? hotelsList : hotelsList.filter((h) => h.category === filter);
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
              ...
              <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filtered.map((hotel) => (
                  <article key={hotel.id || hotel.name} className="bg-white rounded-3xl overflow-hidden transform transition duration-300 hover:-translate-y-2 shadow-md" style={{ boxShadow: '0 8px 30px rgba(2,6,23,0.06)' }}>
                    <div className="h-44 md:h-40 w-full overflow-hidden rounded-t-3xl">
                      <img src={hotel.image || hotel.img || hotel.image_url || 'https://images.unsplash.com/photo-1582719508461-905c673771fd?w=800&q=80'} alt={hotel.name || hotel.title || 'Hotel'} className="w-full h-full object-cover" />
                    </div>

                    <div className="p-5 flex flex-col h-full">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <h3 className="text-xl md:text-2xl font-semibold" style={{ fontFamily: 'Poppins, sans-serif', color: '#0F172A' }}>{hotel.name || hotel.title || 'Hotel'}</h3>
                          <div className="flex items-center gap-2 mt-2 text-sm">
                            <span className="text-yellow-500">⭐</span>
                            <span className="font-medium">{hotel.rating || hotel.stars || '—'}</span>
                          </div>
                        </div>

                        <div className="text-right">
                          <div className="text-2xl font-extrabold" style={{ color: '#2C74B3' }}>{hotel.price ? (typeof hotel.price === 'number' ? `$${hotel.price}` : hotel.price) : (hotel.pricePerNightINR ? `₹${hotel.pricePerNightINR}` : '—')}</div>
                          <div className="text-xs text-gray-500">per night</div>
                        </div>
                      </div>

                      <div className="flex flex-wrap gap-2 mt-4 mb-4">
                        {(hotel.amenities || hotel.best_for || []).slice(0,6).map((a, idx) => (
